...
# from ir_sim.env.env_obs_line import env_obs_line
# from ir_sim.env.env_obs_poly import env_obs_poly
# from ir_sim.env.env_obs_random import env_obs_random
# from ir_sim.env.env_grid import env_grid
# from PIL import Image
...
#                 # obs random
#                 self.obs_random_args = com_list.get('obs_random', dict())
#                 self.obs_random_enable = self.obs_random_args.get('enable', False)
#         else:
#             self.__height = kwargs.get('world_height', 10)
#             self.__width = kwargs.get('world_width', 10)
#             self.step_time = kwargs.get('step_time', 0.1)
...
#             self.vertexes_list = self.obs_polygons_args.get('vertexes_list', [])
#             self.obs_poly_num = self.obs_polygons_args.get('number', 0)
#             self.obs_random_args = kwargs.get('obs_random', dict())
#             self.obs_random_enable = self.obs_random_args.get('enable', False)
...
#         # obs random (initialize after other obstacles to ensure proper collision checking)
#         if self.obs_random_enable:
#             self.components['obs_random'] = env_obs_random(components=self.components, step_time=self.step_time, **{**self.obs_random_args, **kwargs})
#             # Update components with random obstacles for collision checking
#             self._update_components_from_random_obstacles()
#         else:
#             self.components['obs_random'] = None
...
#     def reset_collision_pairs(self):
#         """重置碰撞对信息"""
#         if 'collision_pairs' in self.components:
...
#     def reset(self, **kwargs):
#         # Reset random obstacles if enabled
#         if self.obs_random_enable and self.components['obs_random'] is not None:
#             self.components['obs_random'].reset(**kwargs)
#             self._update_components_from_random_obstacles()
        
#         self.components['robots'].robots_reset(0)
    
#     def _update_components_from_random_obstacles(self):
#         """Update components with obstacles from random obstacle generator."""
#         if self.components['obs_random'] is None:
#             return
            
#         obs_random = self.components['obs_random']
        
#         # Clear any existing random obstacles first (important for reset)
#         self._clear_random_obstacles()
        
#         # Update circle obstacles
#         if hasattr(obs_random, 'obs_circle_list') and obs_random.obs_circle_list:
#             # Merge with existing circle obstacles
#             self.components['obs_circles'].obs_cir_list.extend(obs_random.obs_circle_list)
#             self.obs_cir_list = self.components['obs_circles'].obs_cir_list
        
#         # Update polygon obstacles  
#         if hasattr(obs_random, 'obs_polygon_list') and obs_random.obs_polygon_list:
#             # Merge with existing polygon obstacles
#             self.components['obs_polygons'].obs_poly_list.extend(obs_random.obs_polygon_list)
#             self.obs_poly_list = self.components['obs_polygons'].obs_poly_list
        
#         # Update line obstacles
#         if hasattr(obs_random, 'obs_line_states') and obs_random.obs_line_states:
#             # Merge with existing line obstacles
#             self.components['obs_lines'].obs_line_states.extend(obs_random.obs_line_states)
#             self.obs_line_states = self.components['obs_lines'].obs_line_states
    
#     def _clear_random_obstacles(self):
#         """Clear previously added random obstacles from components."""
#         if not hasattr(self, '_original_obstacle_counts'):
#             # Store original counts on first call
#             self._original_obstacle_counts = {
#                 'circles': len(self.components['obs_circles'].obs_cir_list),
#                 'polygons': len(self.components['obs_polygons'].obs_poly_list),  
#                 'lines': len(self.components['obs_lines'].obs_line_states)
#             }
#         else:
#             # Reset to original counts
#             self.components['obs_circles'].obs_cir_list = self.components['obs_circles'].obs_cir_list[:self._original_obstacle_counts['circles']]
#             self.components['obs_polygons'].obs_poly_list = self.components['obs_polygons'].obs_poly_list[:self._original_obstacle_counts['polygons']]
#             self.components['obs_lines'].obs_line_states = self.components['obs_lines'].obs_line_states[:self._original_obstacle_counts['lines']]
...
#     def get_random_obstacles_info(self):
#         """Get information about randomly generated obstacles for debugging/analysis."""
#         if self.obs_random_enable and self.components['obs_random'] is not None:
#             return self.components['obs_random'].get_all_obstacles_info()
#         return None
